...
public class MoveBehaviourMobile : GenericBehaviour
{
	...
	void Update ()
	{
		...
		// Get hit input.
		if (!hit && VirtualInput.GetButtonDown(hitButton) && behaviourManager.IsCurrentBehaviour(this.behaviourCode) && !behaviourManager.IsOverriding())
		{
			hit = true;
		}
	}
	...
	void JumpManagement()
	{
		...
		if (jump && !behaviourManager.GetAnim.GetBool(jumpBool) && behaviourManager.IsGrounded())
		{
			...
			if(behaviourManager.GetAnim.GetFloat(speedFloat) > -0.1)
			{
				...
				float velocity = 1f * Mathf.Abs(Physics.gravity.y) * jumpHeight;
				velocity = Mathf.Sqrt(velocity);
				behaviourManager.GetRigidBody.AddForce(1.5f* Vector3.up * velocity, ForceMode.VelocityChange);
    ...
    {
		bool now = behaviourManager.GetAnim.GetBool(hitBool);
		if ((behaviourManager.GetAnim.GetFloat(speedFloat)>0.01|| jump) && now )
        {
			hit = false;
			behaviourManager.GetAnim.SetBool(hitBool, false);
			//JumpManagement();
		}
		else if (hit && behaviourManager.IsGrounded())
        {
			now = behaviourManager.GetAnim.GetBool(hitBool);
			behaviourManager.GetAnim.SetBool(hitBool, !now);
			hit = false;
		}
